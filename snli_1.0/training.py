...
class NLIDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = dic[self.df.iloc[index]["gold_label"]]
...
sm = 0
def compute_difficulty(logits, labels):
    #diff_avg = 0
    sm += len(logits)
    diff_sum = 0
    for i in range(len(logits)):
        diff_sum += (1 - logits[i][labels[i]])
    print('$'*150)
    print(diff_sum/len(logits))
...
class ClassificationModel(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        acc, predicted_label = compute_accuracy(logits, batch["labels"])
        compute_difficulty(logits, batch["labels"])
